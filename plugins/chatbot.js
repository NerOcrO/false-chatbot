    ...
    server.route([
      ...
      // Add a message.
      {
        ...
        async handler(request) {
          ...
          const result = await request.mongo.db.collection('chatbot').findOne({ userId: request.payload.user_id, 'thread.id': request.state.id })
          ...
          const result2 = await request.mongo.db.collection('chatbot').findOne({ userId: request.payload.user_id })
